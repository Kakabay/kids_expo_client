...
import { useBurger } from "../../zustand/zusBurger";
...
export const BurgerMenu = () => {
  const burgerOpen = useBurger((state) => state.burger);
  ...
  return (
    <motion.div
      ...
      className={clsx(
        "bg-purple overflow-auto fixed w-full z-[900] top-[74px] bottom-0 left-0 min-h-[100vh] h-full px-4 py-10 flex flex-col gap-10 overflow-y-auto"
      )}
    >
              ...
              <motion.div key={v4()}>
                {item.only ? (
                  ...
                  >
                    ...
                    {!item.only && (
                      <img src={arrow} alt="стрелка" width={20} height={20} />
                    )}
                  </Link>
                ) : (
